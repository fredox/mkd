...
class SqlToDryrunMonad extends SqlToSqlMonad {
    /**
     * @param $data
    ...
    {
        if (!$virtualEnvironment instanceof MysqlEnvironment && !$virtualEnvironment instanceof PgsqlEnvironment) {
            Output::print_msg("[DryRun] VirtualEnvironment (targetEnvironment in Dry run or Source Environment) must be ", "ERROR");
            Output::print_msg("[DryRun] of type MysqlEnvironment or PgsqlEnvironment, [" . get_class($virtualEnvironment) . "] given with environment name: ", "ERROR");
